...
/**
 * A channel is an Procedural Texture Generation interface that can get 0 or more inputs and produces a single output value (either scalar or RGB)
...
public abstract class Channel extends LocalParameterManager {
	...
	/** This is used for rendering and should be set in the constructor
	 * if it is not a normal channel (for example for slow bitmap operations)
	 */
	...
	public static final class CacheEntry {
		...
		public Vector4 sample(float u, float v) {
			...
			//ret.set(values.get(i+0), values.get(i+1), values.get(i+2), values.get(i+3));
		}
	...
	public final Matrix3x3 transformation = new Matrix3x3();
	...
	/**
	 * We just notify all channel change listener if a parameter changed. This method can also be called with
	 * source == null; it then means that a parameter silently changed. Subclasses of channel who
	 * override this method needs to account for source==null and recreate all internal data structures accordingly.
	 */
	public void parameterChanged(AbstractParam source) {
		cacheInvalid = true;
		for (int i = 0; i < changeListener.size(); i++) {
			changeListener.get(i).channelChanged(this);
		}
	}
	...
	public int getNumInputChannels() {
		if (inputChannels == null) return 0;
		return inputChannels.length;
	}
	
	public boolean chechkInputChannels() {
		if (inputChannels == null) return true;
		boolean ret = true;
		for (int i = 0; i < inputChannels.length; i++) {
			if (inputChannels[i] != null) {
				if (!inputChannels[i].chechkInputChannels()) return false;
			} else return false;
	...
	public void setTransformation(Matrix3x3 t) {
		transformation.set(t);
	}
	
	final protected Vector3 transform(float u, float v) {
		Vector3 p = transformation.mult(new Vector3(u, v, 1.0f));
	...
	final public Vector4 valueRGBA(float u, float v) {
	...
	CacheEntry getCache(int xres, int yres) {
		if (cacheInvalid) return null;
		if (cache.xres == xres && cache.yres == yres) return cache;
		return null;
	}
	
	CacheEntry getEmptyCacheEntry(int xres, int yres) {
		if (cache != null && cache.xres == xres && cache.yres == yres) return cache;
		else {
	...
	protected CacheEntry _valuesRGBA_Cached(int xres, int yres) {
		...
		if (getNumInputChannels() > 0) {
			...
			for (int i = 0; i < num; i++) input[i] = inputChannels[i].valuesRGBA_Cached(xres, yres);
		}

		final Vector4 temp = new Vector4();
		for (int i = 0; i < num; i++) inps[i] = new Vector4();
		
		for (int y = 0; y < yres; y++) {
			for (int x = 0; x < xres; x++) {
				int idx = (x + y*xres); 
				Vector3 pos = transform((float)x/(float)xres, (float)y/(float)yres);
				//for (int i = 0; i < num; i++) input[i].sample(inps[i], pos.x, pos.y);
				if (num > 0) cache_function(temp, input, pos.x, pos.y);
				else temp.set(_valueRGBA(pos.x, pos.y));
				out.put(idx, temp);
			}
		}
		return out;
	...
	/**
	 * Computes a image from the given channel;
	 * mode 0: RGB, 1: blended with background; 2: alpha as grayscale
	 */
	private void _computeImage(BufferedImage img, ProgressBarInterface progress, int mode) {
		...
		if (progress != null) progress.startProgress();
		...
		if (useCache && img.getWidth() == 256) {
			...
			for (int y = 0; y < img.getHeight(); y++) {
				...
				for (int x = 0; x < img.getWidth(); x++) {
					final Vector4 col = new Vector4(temp.values.get((x+y*temp.xres)*4+0),temp.values.get((x+y*temp.xres)*4+1),temp.values.get((x+y*temp.xres)*4+2),temp.values.get((x+y*temp.xres)*4+3));
					final Vector3 color = new Vector3();
					//!!UGH TODO: optimize this!!
					if (mode == 0) color.set(col.getVector3());
					else if (mode == 1) {
						...
						color.set(col.w);
					} else Logger.logError(this, "Wrong in computeImage");
		...
		if (progress != null) progress.endProgress();
	}
	
	/**
	 * A Utility method that creates a new image and fills it with the values of the
	 * pattern in [0, 1)
	 * @param xres
	 * @param yres
	 * @return
