...
public final class FilterBlend extends Channel {
	...
	private final Vector4f _function(Vector4f c0, Vector4f c1) {
		float alpha = c1.w;
		if (invertAlpha.get()) alpha = 1.0f - alpha;
		
		final Vector4f color = new Vector4f(c1);
		...
		if (func == 0) { // normal
			...
			color.mul(1/(c0.x + 1), 1/(c0.y + 1), 1/(c0.z + 1), 1);
			...
			color.set(c0).sub(c1).set(new Vector4f(Math.abs(color.x), Math.abs(color.y), Math.abs(color.z), Math.abs(color.w)));
		...
		color.min(new Vector4f(0.0f), new Vector4f(1.0f));
