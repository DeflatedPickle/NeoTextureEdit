...
public final class FilterWarp extends Channel {
	...
	protected void cache_function(Vector4f out, TileCacheEntry[] caches, int localX, int localY, float u, float v) {
        ...
        Vector4f du = caches[1].sample_du(localX, localY);
        Vector4f dv = caches[1].sample_dv(localX, localY);

		// (du.x + du.y + du.z) * (1f / 3f)
		out.set(inputChannels[0].valueRGBA(u + (((du.x + du.y + du.z) * (1f / 3f)) * strength.get()), v + (((dv.x + dv.y + dv.z) * (1f / 3f)) * strength.get())));
	}
	...
	protected Vector4f _valueRGBA(float u, float v) {
		...
		return inputChannels[0].valueRGBA(u + (((du.x + du.y + du.z) * (1f / 3f)) * strength.get()), v + (((dv.x + dv.y + dv.z) * (1f / 3f)) * strength.get()));
	}
