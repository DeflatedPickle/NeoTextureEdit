...
import com.mystictri.neotextureedit.TextureGraphEditorPanel;
...
public final class TextureGraphNode {
    public static final int width = 64 + 24;
    ...
    public Vector<TextureGraphEditorPanel.MiniButton> miniButtons = new Vector<>();
    ...
    public int getHeight() {
        if (folded) return height - 72 + (allConnectionPoints.size() * 8);
        return height;
    }
    ...
    public static class ConnectionPoint {
        ...
        public int getY() {
            if (y > 16 && parent.isFolded()) return (y / 2) + (parent.allConnectionPoints.indexOf(this) * 6); //!!HACK
            return y;
        }
    ...
    public TextureGraphNode(Channel channel) {
        ...
        miniButtons.add(new TextureGraphEditorPanel.MiniButton(posX + 2, posY, 16, 16, "-"));
        miniButtons.add(new TextureGraphEditorPanel.MiniButton(posX + width - 16, posY, 16, 16, "?"));
    }
    ...
    public void setChannel(Channel channel) {
        ...
        // output
        outputConnectionPoint = new ConnectionPoint(this, width - 4, height / 2 - 4, -1, texChannel.getOutputType());
        allConnectionPoints.add(outputConnectionPoint);
        // inputs
        int y = 16;
        for (int i = 0; i < texChannel.getNumInputChannels(); i++) {
            allConnectionPoints.add(new ConnectionPoint(this, -4, y, i, texChannel.getChannelInputType(i)));
            y += 12;
        }
